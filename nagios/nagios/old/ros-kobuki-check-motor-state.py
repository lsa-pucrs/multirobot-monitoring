...
def callback_kobuki(data):
    ...
    while not ready:
        for current in data.status:
            #print current.name
            if current.name == "mobile_base_nodelet_manager: Motor State":
                #print current.name
...
def myhook():
    if kobuki_value == 'Motors Enabled':
        print "OK - Kobuki Motors Enabled"
        exiting(OK)
    #elif kobuki_value == 2:
    #    print "WARNING - Kobuki Watchdog %s" (kobuki_value)
    #    exiting(WARNING)
    else:
        print "CRITICAL - Kobuki Motors Error"
        exiting(CRITICAL)
